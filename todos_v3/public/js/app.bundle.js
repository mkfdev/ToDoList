...
(function () {
  ...
  var allChkinput = document.getElementById('chk-allComplete');
  var tabList = document.querySelectorAll('.nav > li');
  var btnDelete = document.getElementById('btn-removeCompletedTodos');
  ...
  var countLeftNumber = function countLeftNumber() {
    var _todos = void 0;
    return _todos = todos.filter(function (todo) {
      return todo.completed == false;
    }).length;
  };

  var countDeleteNumber = function countDeleteNumber() {
    var _todos = void 0;
    return _todos = todos.filter(function (todo) {
  ...
  document.querySelector('.nav').addEventListener('click', function (e) {
    ...
    [].concat(_toConsumableArray(this.childNodes)).forEach(function (tab) {
      // Skip text node
      if (tab.nodeName === 'LI') {
  ...
  var getTodos = function getTodos() {
    console.log(todos);
    _axios2.default.get('/todos').then(function (rs) {
      todos = rs.data;
      render();
      console.log('[GET]\n', todos);
    }).catch(function (err) {
  ...
  var addTodos = function addTodos() {
    ...
    if (!todos || !todos.length) {
      ...
      todo = { id: lastId(), content: content, completed: false };
    }
  ...
  var removeTodos = function removeTodos(id) {
  ...
  var toggleTodos = function toggleTodos(id) {
    var obj = todos.find(function (todo) {
      return todo.id == id;
    });
    var checked = obj.completed ? false : true;

    _axios2.default.patch('/todos/' + id, { completed: checked }).then(function (res) {
      console.log('[Toggle]\n', res);
      getTodos();
    }).catch(function (err) {
      return console.log(err.response);
  ...
  var toggleAllTodos = function toggleAllTodos(chk) {
    var checkedAll = todos.map(function (todo) {
      return todo.completed == false ? Object.assign({}, todo, { completed: true }) : todo;
    });
    var uncheckedAll = todos.map(function (todo) {
      return todo.completed == true ? Object.assign({}, todo, { completed: false }) : todo;
    });
  ...
  allChkinput.addEventListener('change', function (e) {
